...
def _build_trend_tflite_model(window_size=8,
    ...
    @tf.function(input_signature=[
        ...
        tf.TensorSpec(shape=[window_size], dtype=tf.float32, name="times"),
        tf.TensorSpec(shape=[window_size], dtype=tf.int32,   name="last_preds"),
        tf.TensorSpec(shape=[],            dtype=tf.float32, name="high_thresh"),
        tf.TensorSpec(shape=[],            dtype=tf.float32, name="low_thresh")
    ])
    def _trend_fn(temperatures, times, last_preds, high_thresh, low_thresh):
        """TensorFlow implementation of the enhanced trend & alert logic."""

        # 1) --- Compute current numeric trend ---------------------------------
        ...
        # 2) --- Majority counters over last 8 predictions ----------------------
        majority_req = tf.constant(1, tf.int32)
        heat_cnt  = tf.reduce_sum(tf.cast(tf.equal(last_preds,  1), tf.int32))
        cool_cnt  = tf.reduce_sum(tf.cast(tf.equal(last_preds, -1), tf.int32))
        const_cnt = tf.reduce_sum(tf.cast(tf.equal(last_preds,  0), tf.int32))

        # 3) --- Threshold comparisons -----------------------------------------
        current_temp = temperatures[-1]
        above_thresh = tf.greater(current_temp, high_thresh)
        below_thresh = tf.less(  current_temp, low_thresh)
        inside_thresh = tf.logical_not(tf.logical_or(above_thresh, below_thresh))

        # 4) --- Determine alert / alright status ------------------------------
        def _status_alright():
            return tf.constant(0, tf.int32)  # 0 => alright

        def _status_alert():
            return tf.constant(1, tf.int32)  # 1 => alert

        status = tf.case([
            # High temperature but recent cooling → OK
            (tf.logical_and(above_thresh, tf.greater_equal(cool_cnt, majority_req)), _status_alright),
            # Low temperature but recent heating → OK
            (tf.logical_and(below_thresh, tf.greater_equal(heat_cnt, majority_req)), _status_alright),
            # Inside threshold & trend constant → OK
            (tf.logical_and(inside_thresh,tf.equal(numeric_trend, 0)), _status_alright),
            # Inside threshold & trend heating → OK
            (tf.logical_and(inside_thresh,tf.equal(numeric_trend, 1)), _status_alright),
            # Inside threshold & trend cooling → OK
            (tf.logical_and(inside_thresh,tf.equal(numeric_trend, -1)), _status_alright),
        ], default=_status_alert, exclusive=False)

        # 5) --- Return stacked tensor ----------------------------------------
        return tf.stack([status, numeric_trend])
...
class TemperatureAnalyzer:
    ...
    def predict_trend_tflite(self, temperatures, times):
        ...
        temps_arr = np.array(temperatures[-self.window_size:], dtype=np.float32)
        times_arr = np.array(times[-self.window_size:], dtype=np.float32)

        self.tflite_interpreter.resize_tensor_input(
            self._tflite_input_details[0]['index'], [self.window_size]
        )
        self.tflite_interpreter.resize_tensor_input(
            self._tflite_input_details[1]['index'], [self.window_size]
        )
        self.tflite_interpreter.resize_tensor_input(
            self._tflite_input_details[2]['index'], [self.window_size]
        )
        self.tflite_interpreter.resize_tensor_input(
            self._tflite_input_details[3]['index'], [1]
        )
        self.tflite_interpreter.resize_tensor_input(
            self._tflite_input_details[4]['index'], [1]
        )
        self.tflite_interpreter.allocate_tensors()

        # Pad/trim recent_preds to window_size
        preds_buffer = (self.recent_preds[-self.window_size:] if len(self.recent_preds) >= self.window_size
                        else [0] * (self.window_size - len(self.recent_preds)) + self.recent_preds)

        self.tflite_interpreter.set_tensor(
            self._tflite_input_details[0]['index'], temps_arr
        )
        self.tflite_interpreter.set_tensor(
            self._tflite_input_details[1]['index'], times_arr
        )
        self.tflite_interpreter.set_tensor(
            self._tflite_input_details[2]['index'], np.array(preds_buffer, dtype=np.int32)
        )
        self.tflite_interpreter.set_tensor(
            self._tflite_input_details[3]['index'], np.array([self.high_threshold], dtype=np.float32)
        )
        self.tflite_interpreter.set_tensor(
            self._tflite_input_details[4]['index'], np.array([self.low_threshold], dtype=np.float32)
        )

        # Inference
        self.tflite_interpreter.invoke()
        result = self.tflite_interpreter.get_tensor(
            self._tflite_output_details[0]['index']
        )

        status = int(result[0])
        numeric_trend = int(result[1])

        status_str = {0: "alright", 1: "alert"}.get(status, "unknown")
        trend_str = {1: "Heating", -1: "Cooling", 0: "Constant"}.get(numeric_trend, "Unknown")

        return {"status": status_str, "trend": trend_str}
