...
def build_trend_tflite_model(window_size=8,
    ...
    @tf.function(input_signature=[
        tf.TensorSpec(shape=[1, window_size+2], dtype=tf.float32, name="temperatures"),
    ])
    def _trend_fn(temperatures):
        """TensorFlow implementation of the enhanced trend & alert logic."""
        # Remove the leading batch dimension (always size 1)
        temperatures = tf.squeeze(temperatures, axis=0)

        high_thresh = temperatures[-1]
        low_thresh = temperatures[-2]
        ...
        # Convert scalar status (0 = safe, 1 = alert) into a 1-D vector [safe, alert]
        # This yields an output tensor of shape [2] – a single dimension with length >1.
        status_safe  = tf.cast(1 - status, output_dtype)
        status_alert = tf.cast(status,       output_dtype)
        status_vec   = tf.stack([status_safe, status_alert])  # shape = [2]
        # Add batch dimension so final output shape is [1, 2]
        status_vec   = tf.expand_dims(status_vec, axis=0)
        return status_vec
